...
GType
gossip_contact_list_get_type (void)
{
	static GType type = 0;

	if (!type) {
		static const GTypeInfo type_info = {
			sizeof (GossipContactListClass),
...
contact_list_base_init (gpointer klass)
{
	...
	if (!initialized) {
		g_signal_new ("contact_added",
			      ...
			      G_SIGNAL_RUN_LAST,
			      G_STRUCT_OFFSET (GossipContactListClass,
					       contact_added),
			      NULL, NULL,
...
GList *
gossip_contact_list_get_contacts (GossipContactList *list)
{
	g_return_val_if_fail (GOSSIP_IS_CONTACT_LIST (list), NULL);

	if (GOSSIP_CONTACT_LIST_GET_CLASS (list)->get_contacts) {
		return GOSSIP_CONTACT_LIST_GET_CLASS (list)->get_contacts (list);
	}
