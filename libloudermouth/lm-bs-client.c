...
static gboolean read_events_cb            (GIOChannel   *source, 
					   GIOCondition  condition,
					   gpointer      user_data);
static gboolean write_events_cb           (GIOChannel   *source, 
					   GIOCondition  condition,
					   gpointer      user_data);
static guint    client_add_watch          (LmBsClient   *client,
					   GIOChannel   *channel,
					   GIOCondition  condition,
					   GIOFunc       func);
static void     client_remove_watch       (LmBsClient   *client);
static gboolean connect_timeout           (LmBsClient   *client);
static gboolean connection_failed         (LmBsClient   *client);
static gboolean connection_succeeded      (LmBsClient   *client);
static gboolean client_get_connect_events (GIOChannel   *source, 
					   GIOCondition  condition,
					   gpointer      user_data);
static gint     client_try_to_connect     (LmBsClient   *client);
static gboolean fill_server_address_info  (LmBsClient   *client);
static void     free_client               (LmBsClient   *client);


static gboolean
read_events_cb (GIOChannel   *source,
		GIOCondition  condition,
		gpointer      user_data)
{
	LmBsClient *client;
...
static gboolean
write_events_cb (GIOChannel   *source,
		 GIOCondition  condition,
		 gpointer      user_data)
{
	LmBsClient   *client;
...
static guint
client_add_watch (LmBsClient   *client,
		  GIOChannel   *channel,
		  GIOCondition  condition,
		  GIOFunc       func)
{
...
static void
client_remove_watch (LmBsClient *client)
{
	GSource *source;
...
static gboolean
connect_timeout (LmBsClient *client)
{
...
static gboolean
connection_failed (LmBsClient *client)
{
	LmCallback *cb;
...
static gboolean 
connection_succeeded (LmBsClient *client)
{
	LmCallback *cb;

	client_remove_watch (client);

	if (client->status == C_STATUS_CONNECTING) {
		client->status = C_STATUS_CONNECTED;
	}
...
static gboolean 
client_get_connect_events (GIOChannel   *source,
			   GIOCondition  condition,
			   gpointer      user_data)
{
	LmBsClient *client;
	...
	result = client_try_to_connect (client);
	if (result < 0) {
		err = _lm_sock_get_last_error ();
		if (_lm_sock_is_blocking_success (err)) {
			connection_succeeded (client);
		}
		if (client->status == C_STATUS_CONNECTING && 
...
static gint
client_try_to_connect (LmBsClient *client)
{
	gint result;
...
static gboolean
fill_server_address_info (LmBsClient *client)
{
	struct addrinfo  req;
	struct addrinfo *ans;
	int              result;
...
lm_bs_client_connect (LmBsClient *client)
{
	...
	if (!fill_server_address_info (client)) {
		g_log (LM_LOG_DOMAIN, LM_LOG_LEVEL_NET,
		       "Invalid address %s.\n",
		       client->host);
		return -1;
	...
	_lm_sock_set_blocking (client->fd, FALSE);
	client->status = C_STATUS_CONNECTING;
	client->io_watch = client_add_watch (client,
					     client->io_channel,
					     IO_CONNECT_FLAGS,
				             (GIOFunc) client_get_connect_events);

	source = g_timeout_source_new (CONNECT_TIMEOUT);
	g_source_set_callback (source,
			       (GSourceFunc) connect_timeout,
			       client,
			       NULL);
...
lm_bs_client_stop (LmBsClient *client)
{
	...
	client->io_channel = NULL;
	client->status = C_STATUS_INTERRUPTED;
